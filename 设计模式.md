...
### 访问者模式：将数据结构和访问操作分离，造就一个访问者

新增操作时无需修改元素类，只需新增访问者，从而解决 “数据结构稳定但操作多变” 的问题。

访问者模式的核心是 “双分派”，即**操作的选择由 “访问者类型” 和 “元素类型” 共同决定**：

accept/ visitor 

>核心流程： 调用你类里面的所有element元素接收他的visitor，然后visitor去调用visit方法访问元素内部内容；
...
private:
    std::string content_; // 文字内容
    int length_;          // 长度（字数）
public:
    Text(std::string content, int length) : content_(std::move(content)), length_(length) {}

    // 获取属性（供访问者使用）
    std::string getContent() const { return content_; }
    int getLength() const { return length_; }

    // 接受访问者：调用访问者的visit(Text*)方法
...
private:
    std::string title_;  // 表格标题
    int rows_;           // 行数
    int cols_;           // 列数
public:
    Table(std::string title, int rows, int cols) 
        : title_(std::move(title)), rows_(rows), cols_(cols) {}
...
// 4. 具体访问者1：打印访问者（PrintVisitor）
class PrintVisitor : public Visitor {
public:
    void visit(Text* text) override {
        std::cout << "打印文字：内容='" << text->getContent() << "', 长度=" << text->getLength() << "字" << std::endl;
    }

    void visit(Image* image) override {
        std::cout << "打印图片：名称='" << image->getName() << "', 尺寸=" << image->getWidth() << "x" << image->getHeight() << std::endl;
    }

    void visit(Table* table) override {
        std::cout << "打印表格：标题='" << table->getTitle() << "', 大小=" << table->getRows() << "行x" << table->getCols() << "列" << std::endl;
    }
};

// 具体访问者2：PDF导出访问者（PdfExportVisitor）
class PdfExportVisitor : public Visitor {
public:
    void visit(Text* text) override {
        std::cout << "导出文字到PDF：内容='" << text->getContent() << "'" << std::endl;
    }

    void visit(Image* image) override {
        std::cout << "导出图片到PDF：名称='" << image->getName() << "'（压缩尺寸）" << std::endl;
    }

    void visit(Table* table) override {
        std::cout << "导出表格到PDF：标题='" << table->getTitle() << "'（自动调整列宽）" << std::endl;
    }
};

// 5. 对象结构（ObjectStructure）：文档（Document）
class Document {
private:
    std::vector<std::unique_ptr<Element>> elements_; // 存储元素的集合
...
int main() {
    ...
    // 向文档添加元素
    document.addElement(std::make_unique<Text>("访问者模式示例", 7));
    document.addElement(std::make_unique<Image>("流程图.png", 800, 600));
    document.addElement(std::make_unique<Table>("角色说明", 5, 2));
...
### 中介者模式：多个同事之间不直接交互，而是通过中介者去进行交互，降低耦合度

同事之间是不需要互相知道的；

中介者模式的解决方案：

- 所有同事对象**只与中介者通信**，不直接交互；
- 中介者**集中管理所有同事间的交互规则**（如 “用户 A 发消息时，中介者转发给其他所有用户”）；
- 同事对象的增减或交互规则的修改，只需调整中介者，无需修改其他同事。

组成：

1. 中介者的接口
2. 中介者的实现；需要保存他的客户的list
3. 客户的抽象接口
4. 客户具体实现，需要在构造时传入中介者的实现，
...
class Media{
	void registerClient()=0;
	void sendMsg()=0;
};


class Media2: public Media{
	std::list<User*>
...
### 解释器式：**定义一种语言的语法规则，并构建一个解释器来解释该语言中的句子**。
...
解释‘模式通过**将语法规则抽象为对象**，用 “组合模式” 构建语法树，实现语法与解析逻辑的分离，让扩展和维护更简单。
...
// 2. 终结符表达式2：变量表达式（处理变量，从上下文获取值）
class VariableExpression : public Expression {
private:
    std::string var_name_;
public:
    explicit VariableExpression(std::string var_name) : var_name_(std::move(var_name)) {}

    int interpret(const std::unordered_map<std::string, int>& context) const override {
        // 从上下文查找变量，未定义则抛异常
        auto it = context.find(var_name_);
        if (it == context.end()) {
            throw std::invalid_argument("未定义变量: " + var_name_);
        }
        return it->second;
    }
...
// 3. 非终结符表达式1：加法表达式（a + b）
class AddExpression : public Expression {
private:
    std::unique_ptr<Expression> left_;  // 左子表达式
    std::unique_ptr<Expression> right_; // 右子表达式
public:
...
public:
    ...
    int interpret(const std::unordered_map<std::string, int>& context) const override {
        int right_val = right_->interpret(context);
        if (right_val == 0) {
            throw std::invalid_argument("除零错误");
        }
        return left_->interpret(context) / right_val;
    }
...
// 4. 上下文：存储变量映射和解析状态（简化版，聚焦核心功能）
struct Context {
    std::unordered_map<std::string, int> variables; // 变量名 -> 值
    size_t parse_pos = 0;                            // 当前解析位置（供解析器使用）
};

// 5. 解析器（客户端的辅助工具，负责构建语法树）
class ExpressionParser {
private:
    std::string expr_; // 待解析的表达式字符串
    Context context_;  // 解析上下文

    // 跳过空格
    void skipWhitespace() {
        while (context_.parse_pos < expr_.size() && std::isspace(expr_[context_.parse_pos])) {
            context_.parse_pos++;
        }
    }
    ...
    std::string parseVariable() {
        skipWhitespace();
        if (context_.parse_pos >= expr_.size() || !std::isalpha(expr_[context_.parse_pos])) {
            throw std::invalid_argument("无效变量名，需以字母开头");
        }
        std::string var_name;
        while (context_.parse_pos < expr_.size() && (std::isalnum(expr_[context_.parse_pos]) || expr_[context_.parse_pos] == '_')) {
            var_name += expr_[context_.parse_pos];
            context_.parse_pos++;
        }
        return var_name;
    }
    ...
    int parseNumber() {
        skipWhitespace();
        if (context_.parse_pos >= expr_.size() || !std::isdigit(expr_[context_.parse_pos])) {
            throw std::invalid_argument("无效数字");
        }
        int num = 0;
        while (context_.parse_pos < expr_.size() && std::isdigit(expr_[context_.parse_pos])) {
            num = num * 10 + (expr_[context_.parse_pos] - '0');
            context_.parse_pos++;
        }
        return num;
    }

    // 解析因子（数字、变量或带括号的表达式，优先级最高）
    std::unique_ptr<Expression> parseFactor() {
        skipWhitespace();
        if (context_.parse_pos >= expr_.size()) {
            throw std::invalid_argument("表达式不完整");
        }

        // 处理括号：( 表达式 )
        if (expr_[context_.parse_pos] == '(') {
            context_.parse_pos++; // 跳过 '('
            auto sub_expr = parseExpression(); // 递归解析括号内的表达式
            skipWhitespace();
            if (context_.parse_pos >= expr_.size() || expr_[context_.parse_pos] != ')') {
        ...
        // 处理变量（以字母开头）
        else if (std::isalpha(expr_[context_.parse_pos])) {
            return std::make_unique<VariableExpression>(parseVariable());
        }
        // 处理数字（以数字开头）
        else {
            return std::make_unique<NumberExpression>(parseNumber());
        }
    }

    // 解析项（处理乘除，优先级高于加减）
    std::unique_ptr<Expression> parseTerm() {
        auto left = parseFactor(); // 先解析第一个因子

        skipWhitespace();
        // 循环处理后续的 * 或 /
        while (context_.parse_pos < expr_.size()) {
            char op = expr_[context_.parse_pos];
            if (op != '*' && op != '/') {
                break; // 不是乘除，退出循环（交给 parseExpression 处理加减）
            }
            context_.parse_pos++; // 跳过运算符

            auto right = parseFactor(); // 解析运算符右侧的因子
            // 根据运算符组合表达式
            if (op == '*') {
    ...
    std::unique_ptr<Expression> parseExpression() {
        auto left = parseTerm(); // 先解析第一个项（乘除组合）
        ...
        while (context_.parse_pos < expr_.size()) {
            char op = expr_[context_.parse_pos];
            if (op != '+' && op != '-') {
                break; // 不是加减，退出循环（表达式解析完成）
            }
            context_.parse_pos++; // 跳过运算符

            auto right = parseTerm(); // 解析运算符右侧的项
            // 根据运算符组合表达式
            if (op == '+') {
...
public:
    explicit ExpressionParser(std::string expr) : expr_(std::move(expr)) {
        // 预处理：移除所有空格（简化解析逻辑）
        expr_.erase(std::remove_if(expr_.begin(), expr_.end(), ::isspace), expr_.end());
    }

    // 绑定变量（如 a=2, b=4）
    void bindVariable(const std::string& var_name, int value) {
        context_.variables[var_name] = value;
    }

    // 解析表达式，返回语法树的根节点
    std::unique_ptr<Expression> parse() {
        context_.parse_pos = 0; // 重置解析位置
        auto root = parseExpression();
        // 解析完成后，若还有未处理的字符，说明语法错误
        if (context_.parse_pos != expr_.size()) {
            throw std::invalid_argument("语法错误：多余字符 '" + std::string(1, expr_[context_.parse_pos]) + "'");
        }
        return root;
    }
...
int main() {
    try {
        // 测试1：纯数字表达式 (3 + 5) * 2
        ExpressionParser parser1("(3+5)*2");
        auto expr1 = parser1.parse();
        std::cout << "表达式结构：" << expr1->toString() << std::endl;
        std::cout << "计算结果：" << expr1->interpret(parser1.getContext().variables) << "\n" << std::endl;

        // 测试2：含变量的表达式 a*(b - c)，绑定变量 a=2, b=10, c=3
        ExpressionParser parser2("a*(b-c)");
        parser2.bindVariable("a", 2);
        parser2.bindVariable("b", 10);
        parser2.bindVariable("c", 3);
        auto expr2 = parser2.parse();
        std::cout << "表达式结构：" << expr2->toString() << std::endl;
        std::cout << "计算结果：" << expr2->interpret(parser2.getContext().variables) << "\n" << std::endl;

        // 测试3：混合运算 10/(2+3) + 4*2
        ExpressionParser parser3("10/(2+3)+4*2");
        auto expr3 = parser3.parse();
        std::cout << "表达式结构：" << expr3->toString() << std::endl;
        std::cout << "计算结果：" << expr3->interpret(parser3.getContext().variables) << std::endl;
